...
| `check-rig-freshness.sh` | `task rig:smoke` (reuse-existing-rig path), `run-golden.sh` | Stale-jar guard (#44). `task dev:titan` bakes the checkout HEAD into the titan-server image (`GIT_SHA` build-arg → OCI label `org.opencontainers.image.revision` + `/app/TITAN_GIT_SHA`). This probe `docker inspect`s the label and compares it to `git rev-parse HEAD`: on mismatch — or when the label is missing/`unknown`, i.e. the image was built via a direct `docker compose up --build titan-server` — it prints a LOUD `STALE RIG` warning and emits `true` (→ `"rigShaMismatch":true` in the telemetry). Warning only, always exit 0: intentional drift mid-bisect is legitimate. Skip: `RIG_SMOKE_SKIP_FRESHNESS=1`. |
